...
namespace NetAF.Tests.Interpretation
{
    ...
    public class FrameCommandInterpreter_Tests
    {
        ...
        {
            var interpreter = new FrameCommandInterpreter();
            var game = Game.Create(new GameInfo(string.Empty, string.Empty, string.Empty), string.Empty, AssetGenerator.Retained(null, new PlayableCharacter(string.Empty, string.Empty)), GameEndConditions.NoEnd, GameConfiguration.Default).Invoke();

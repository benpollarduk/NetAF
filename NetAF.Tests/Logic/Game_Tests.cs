    ...
    public class Game_Tests
    {
        ...
        public void GivenSimpleGame_WhenChangeModeToRegionMap_ThenNoExceptionThrown()
        {
            Assertions.NoExceptionThrown(() =>
            {
                ...
                game.ChangeMode(new RegionMapMode(RegionMapMode.Player, RegionMapDetail.Normal, new RegionMapCommandInterpreter()));
            });
        }
        ...
        [TestMethod]
        public void GivenStartModeTitleScreen_WhenUpdate_ThenCompletedIsTrue()
        {
            var room = new Room(string.Empty, "A room", "Entering a room", items: [new Item("a", "b")], exits: [new Exit(Direction.North, false, new("c"))]);
            room.AddCharacter(new("d", "e"));
            RegionMaker regionMaker = new(string.Empty, string.Empty);
            ...
            OverworldMaker overworldMaker = new(string.Empty, string.Empty, regionMaker);
            PlayableCharacter player = new("A", string.Empty);
            var game = Game.Create(new(string.Empty, string.Empty, string.Empty), string.Empty, AssetGenerator.Retained(overworldMaker.Make(), player), GameEndConditions.NoEnd, new GameConfiguration(new TestConsoleAdapter(), FrameBuilderCollections.Console, new Size(80, 50), new TitleMode(), FinishModes.ReturnToTitleScreen)).Invoke();
        ...
        [TestMethod]
        public void GivenStartModeScene_WhenUpdate_ThenCompletedIsTrue()
        {
            var room = new Room(string.Empty, "A room", "Entering a room", items: [new Item("a", "b")], exits: [new Exit(Direction.North, false, new("c"))]);
            room.AddCharacter(new("d", "e"));
            RegionMaker regionMaker = new(string.Empty, string.Empty);
            ...
            OverworldMaker overworldMaker = new(string.Empty, string.Empty, regionMaker);
            PlayableCharacter player = new("A", string.Empty);
            var game = Game.Create(new(string.Empty, string.Empty, string.Empty), string.Empty, AssetGenerator.Retained(overworldMaker.Make(), player), GameEndConditions.NoEnd, new GameConfiguration(new TestConsoleAdapter(), FrameBuilderCollections.Console, new Size(80, 50), new SceneMode(new SceneCommandInterpreter()), FinishModes.ReturnToTitleScreen)).Invoke();

        ...
        public NonPlayableCharacter Instantiate()
        {
            ...
            saria = new NonPlayableCharacter(Name, Description, conversation: conversation, interaction: item =>
            {
                ...
                if (Rupee.Name.EqualsIdentifier(item.Identifier) && key != null)
                {
                    ...
                    room.AddItem(key);
                    return new(InteractionEffect.ItemExpired, item, $"{saria.Identifier.Name} looks excited! \"Thanks Link, here take the Tail Key!\" Saria put the Tail Key down, awesome!");
                }

                if (Shield.Name.EqualsIdentifier(item.Identifier))
                {
                    return new(InteractionEffect.NeitherItemOrTargetExpired, item, $"{saria.Identifier.Name} looks at your shield, but seems pretty unimpressed.");
                }
                ...
                if (Sword.Name.EqualsIdentifier(item.Identifier) && saria.IsAlive)
                {
                    ...
                    if (!saria.HasItem(key))
                        return new(InteractionEffect.NeitherItemOrTargetExpired, item, $"You strike {saria.Identifier.Name} in the face with the sword and she falls down dead.");

                    saria.RemoveItem(key);
                    room.AddItem(key);

                    return new(InteractionEffect.NeitherItemOrTargetExpired, item, $"You strike {saria.Identifier.Name} in the face with the sword and she falls down dead. When she fell you saw something drop to out of her hand, it looked like a key...");
                }

        ...
        public Room Instantiate()
        {
            ...
            room = new Room(Name, string.Empty, [new Exit(Direction.West), new Exit(Direction.North, true)], interaction: item =>
            {
                if (item != null && ConchShell.Name.EqualsExaminable(item))
                {
                    ...
                    return new(InteractionEffect.ItemExpired, item, "You blow into the Conch Shell. The Conch Shell howls, the  bats leave! Conch shell crumbles to pieces.");
                }

                if (item != null && Knife.Name.EqualsExaminable(item))
                    return new(InteractionEffect.NeitherItemOrTargetExpired, item, "You slash wildly at the bats, but there are too many. Don't aggravate them!");
